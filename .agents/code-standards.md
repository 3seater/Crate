...
- **Web (`apps/web`):** Do not use the HTML `title` attribute for hover tooltips unless the task explicitly asks for tooltips. Prefer `aria-label` for accessible names; use `@/components/ui/tooltip` when styled tooltips are requested. Keep `<iframe title>` and SVG `<title>` for a11y.
...
- **tRPC procedures** must throw `TRPCError` or `createAppError` (not raw `Error` or `ApiError`)
- Use `createAppError` when why/fix/link improve UX (auth, trading); use `TRPCError` for simple cases
