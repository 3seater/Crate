...
const inputGroupAddonVariants = cva(
  "flex h-auto cursor-text select-none items-center justify-center gap-1 py-2 font-medium text-muted-foreground text-xs/relaxed **:data-[slot=kbd]:rounded-[calc(var(--radius-sm)-(2px*var(--radius-scale)))] **:data-[slot=kbd]:bg-muted-foreground/10 **:data-[slot=kbd]:px-1 **:data-[slot=kbd]:text-[0.625rem] group-data-[disabled=true]/input-group:opacity-50 [&>svg:not([class*='size-'])]:size-3.5",
  {
    variants: {
      align: {
        "inline-start":
...
function InputGroupAddon({
  ...
  return (
    // biome-ignore lint/a11y/noNoninteractiveElementInteractions: click-to-focus pattern delegates to child input
    // biome-ignore lint/a11y/useSemanticElements: shadcn/ui pattern — div with role="group" for addon grouping
    <div
...
const inputGroupButtonVariants = cva(
  "flex items-center gap-2 rounded-md text-xs/relaxed shadow-none",
  {
    variants: {
      size: {
        xs: "h-5 gap-1 rounded-[calc(var(--radius-sm)-(2px*var(--radius-scale)))] px-1 [&>svg:not([class*='size-'])]:size-3",
        sm: "",
