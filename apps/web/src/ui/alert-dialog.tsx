...
function AlertDialogOverlay({
  ...
  return (
    <AlertDialogPrimitive.Backdrop
      className={cn(
        "data-closed:fade-out-0 data-open:fade-in-0 fixed inset-0 isolate z-50 bg-black/70 backdrop-blur-[2px] duration-100 data-closed:animate-out data-open:animate-in",
        className
      )}
  ...
  return (
    <AlertDialogPortal>
      ...
      <AlertDialogPrimitive.Popup
        className={cn(
          "data-closed:fade-out-0 data-open:fade-in-0 data-closed:zoom-out-95 data-open:zoom-in-95 group/alert-dialog-content fixed top-1/2 left-1/2 z-50 grid w-full -translate-x-1/2 -translate-y-1/2 gap-3 rounded-xl border border-border-frame bg-background p-4 outline-none ring-0 duration-100 data-[size=default]:max-w-xs data-[size=sm]:max-w-64 data-closed:animate-out data-open:animate-in data-[size=default]:sm:max-w-sm",
          className
        )}
        data-size={size}
